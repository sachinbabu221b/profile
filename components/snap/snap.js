...
function updatePartialCanvas(canvas, partialCanvas, distribution) {
  ...
  for (let i = 0; i < pixelArr.length; i += IMAGE_CHANNEL) {
    ...
    const targetCanvas =
      imageDataArray[
        weightedRandomDistrib(hPos, vPos, canvasNum, distribution)
      ];
...
function DustEffect({ src, show, option, imgProps, ...props }) {
  ...
  const partialCanvas = useRef(
    Array.from({ length: option.canvasNum }, () => React.createRef())
  );
      ...
      if (pc.current) {
        updateStyle(pc.current, {
          transition: [
            ...
            `transform ${
              option.baseDuration + option.innerTimeoutDelay * i
            }ms cubic-bezier(0.55, 0.085, 0.68, 0.53) ${delay}ms`,
            `opacity ${
              option.baseDuration + option.innerTimeoutDelay * i
            }ms cubic-bezier(0.755, 0.05, 0.855, 0.06) ${delay}ms`,
          ],
          filter: `blur(${option.blur}px)`,
          opacity: "0",
        ...
        updateStyle(pc.current, {
          transition: [
            `filter ${
              ...
              option.innerTimeoutDelay * j +
              (option.innerTimeoutDelay - option.outerTimeoutDelay) * i
            }ms`,
            `transform ${
              option.baseDuration +
              (option.innerTimeoutDelay - option.outerTimeoutDelay) * i
  ...
  const handleImageLoad = useCallback(
    (e) => {
      ...
      if (canvas) {
        ...
        updatePartialCanvas(
          canvas,
          partialCanvas.current,
          option.distributionFunc
        );
        setConverted(true);
      }
    ...
    <>
      ...
      >
        ...
        <img
          ...
          onLoad={handleImageLoad}
        />
        {!converted && <canvas ref={canvasRef} style={{ display: "none" }} />}
